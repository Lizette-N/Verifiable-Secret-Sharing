...
def add_time(metrics, name, elapsed):
...
def print_metrics(metrics):
    print("\nRuntime analysis")
    print("----------------")
    print(f"Total dealing time:        {metrics['dealing']:.6f} seconds")
    print(f"Total verification time:   {metrics['verification']:.6f} seconds")
    print(f"Total reconstruction time: {metrics['reconstruction']:.6f} seconds")
...
def sharing_phase(metrics):
    ...
    print(f"PC.Setup time:              {setup_end - setup_start:.6f} seconds")
    print(f"Key generation time:        {keygen_end - keygen_start:.6f} seconds")
    make_malicious(nodes)

    dealing_start = time.perf_counter()
    v, w = PC.Commit(pp, poly, n)
    shares = send_shares(pp, v, w, poly, n)
    add_time(metrics, "dealing", time.perf_counter() - dealing_start)
    ...
    add_time(metrics, "verification", time.perf_counter() - verification_start)
...
def algorithm1():
    ...
    sharing_total_start = time.perf_counter()
    pp, t, q, nodes = sharing_phase(metrics)
    sharing_total_end = time.perf_counter()

    reconstruction_start = time.perf_counter()
    secret = reconstruction_phase(pp, t, q, nodes)
    add_time(metrics, "reconstruction", time.perf_counter() - reconstruction_start)
    ...
    print("\nExtra runtime debug")
    print("----------------")
    print(f"Full sharing_phase time:    {sharing_total_end - sharing_total_start:.6f} seconds")
    print(f"Measured dealing time:      {metrics['dealing']:.6f} seconds")
    print(f"Measured verification time: {metrics['verification']:.6f} seconds")
    print(f"Measured reconstruction:    {metrics['reconstruction']:.6f} seconds")
    print(f"Unmeasured overhead:        {(total_end - total_start) - sum(metrics.values()):.6f} seconds")
